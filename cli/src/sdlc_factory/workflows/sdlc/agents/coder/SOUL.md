...
19. **Clean Infrastructure State**: When starting or testing against live infrastructure, ALWAYS ensure you tear down any stale containers before rebuilding. You MUST use the command `docker compose down && docker compose up -d` (or include `--build` if necessary) to guarantee you are testing against fresh infrastructure.
