...
class SdlcWorkflow(WorkflowPlugin):
    ...
    def schemas(self) -> Dict[str, Dict[str, Any]]:
        return {
            "PLANNING": {"file": "spec_payload.json", "required_artifacts": ["docs/PROD_SPEC.md"], "schema": {"oneOf": [{"type": "object", "required": ["status", "phase_completed", "product_spec_hash", "identified_modules"], "properties": {"status": {"const": "success"}, "phase_completed": {"const": "PLANNING"}, "product_spec_hash": {"type": "string"}, "identified_modules": {"type": "array", "items": {"type": "string"}}}}, {"type": "object", "required": ["status", "phase_completed", "rfc_file_path"], "properties": {"status": {"const": "rfc_requested"}, "phase_completed": {"const": "PLANNING"}, "rfc_file_path": {"const": "docs/RFC.md"}}}]}},
            "ARCHITECTURE": {"file": "arch_payload.json", "required_artifacts": ["docs/API_CONTRACTS.md"], "schema": {"oneOf": [{"type": "object", "required": ["status", "phase_completed", "api_contracts_hash", "vertical_slices"], "properties": {"status": {"const": "success"}, "phase_completed": {"const": "ARCHITECTURE"}, "api_contracts_hash": {"type": "string"}, "vertical_slices": {"type": "array", "items": {"type": "object", "required": ["module_name", "assigned_agent"], "properties": {"module_name": {"type": "string"}, "assigned_agent": {"type": "string"}, "context_queries": {"type": "array", "items": {"type": "string"}}}}}}},{"type": "object","required": ["status", "phase_completed", "rfc_file_path"],"properties": {"status": {"const": "rfc_requested"},"phase_completed": {"const": "ARCHITECTURE"},"rfc_file_path": {"const": "docs/RFC.md"}}}]}},
    ...
    def get_phase_context(self, phase: str, workspace: Path) -> str:
        ...
        phase_files = {
            ...
            "TEST_DESIGN": ["handoff/regression_report.json"],
    ...
    def _consolidate_integration(self, ws: Path, task_id: str):
        ...
        global_logger.info(f"📦 [CONSOLIDATION] Bundled all into {parent_id}.", extra={"color": typer.colors.GREEN})
