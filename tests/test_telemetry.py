...
def test_setup_telemetry_disabled(mocker):
    ...
    mock_instrument = mocker.patch("sdlc_factory.telemetry.OpenAIInstrumentor.instrument")
...
def test_setup_telemetry_fails_gracefully(mocker):
    mocker.patch("sdlc_factory.telemetry._TELEMETRY_INITIALIZED", False)
    mock_instrument = mocker.patch("sdlc_factory.telemetry.OpenAIInstrumentor.instrument", side_effect=Exception("Failed"))
    setup_telemetry({"tracing_enabled": True})
